...
# 设置 Tesseracts OCR 的路径
pytesseract.pytesseract.tesseract_cmd = r'E:\Tesseract-OCR\tesseract.exe'

# 定义屏幕区域的坐标
first_number_box = (255,428,150,100 )  # 第一个数字的区域
second_number_box = (520,428 ,150,100)  # 第二个数字的区域
...
def draw_greater_than():
    pyautogui.moveTo(200, 888,duration=0)
    pyautogui.dragTo(x=240, y=1000, duration=0.1, button='left')
    pyautogui.dragTo(x=200, y=1000, duration=0.1, button='left')

      # 移动到绘制位置
    #pyautogui.mouseDown()  # 按下鼠标
    #pyautogui.moveTo(210, 900, duration=0)  # 移动到大于符号的尖端位置
    #pyautogui.moveTo(200, 920, duration=0.2)  # 移动到结束位置
    #pyautogui.mouseUp()  # 放开鼠标

# 绘制小于符号
def draw_less_than():
    pyautogui.moveTo(200, 888,duration=0)
    pyautogui.dragTo(x=160, y=1000,duration=0.1, button='left')
    pyautogui.dragTo(x=200, y=1000, duration=0.1, button='left')

    #pyautogui.moveTo(200, 888)  # 移动到绘制位置
    #pyautogui.mouseDown()  # 按下鼠标
    #pyautogui.moveTo(190, 900, duration=0)  # 移动到小于符号的尖端位置
    #pyautogui.moveTo(200, 920, duration=0.2)  # 移动到结束位置
    #pyautogui.mouseUp()  # 放开鼠标
